...
def Rayleigh_plus_line(x, a , b , c):
    return Rayleigh(x, c) + line(x, a, b)
...
def compute_sigma_and_norm(data, bins, a, b):
    ...
    c, _ = curve_fit(Rayleigh, midbins, fr, p0=[np.max(fr)], maxfev = 6000)# , sigma = erreurs)
    c = c[0]
    mse = 1 - MSE(fr, Rayleigh(midbins, c))
    if (c>10000): #Constante trop grande ( to be replaced by np.max(histo)???)
        # print("c and s too big:\nc=",c,"\ns=",s)
        c = np.nan
        mse = np.nan
    return c  , mse, fr

def CR(x , a , b , c ): #Inégrale analytique
    return (a/2 * x**2 + b*x) / (a/2 * x**2 + b*x + c * (1 - np.exp(-(x**2/(2)))) )

def all_in_one(data, data_shifted, bins):
    a,b = compute_a_b(data_shifted, bins)
    c,mse, fr = compute_sigma_and_norm(data, bins,a,b)
    return a , b , c  , 1 - CR(data,a,b,c) , mse
